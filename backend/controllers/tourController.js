...
// Create new Tour
exports.createOneTour = factory.createOne(Tour);

exports.getTop5CheapTours = (req, res, next) => {
  req.query.limit = '5';
  req.query.sort = '-ratingsAverage,price';
  req.query.fields =
    'name,duration,summary,price,ratingsAverage,createdAt,difficulty';
  next();
};
...
exports.getTourWithin = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) {
    return next(
      new AppError('please provide valid lat lng in formt lat,lng', 400),
    );
  }
  ...
  const tours = await Tour.find({
    ...
    .select('name -_id') // name فقط
    .setOptions({ skipPopulate: true })
    .lean({ virtuals: false }); // يمنع الحقول الافتراضية
